...
def resolve_input_path(cli_input: str | None) -> str:
    ...
    Priority:
      ...
      3) data/raw or data/sample (fallback)
    Ensure s3a:// scheme for Spark.
    """
    ...
    if cli_input:
        ...
        # local fallback (use sample if present)
        path = "data/raw" if Path("data/raw").exists() else "data/sample"
...
def make_duration_density_for_largest_cluster(df_timeline: pd.DataFrame, out_png: Path):
    # choose cluster with most applications
    top_cluster = (
        df_timeline["cluster_id"].value_counts().sort_values(ascending=False).index[0]
    )
    sub = df_timeline[df_timeline["cluster_id"] == top_cluster].copy()
    # duration seconds (already provided), plot on log scale
    plt.figure(figsize=(10, 5))
...
def main():
    ...
    parser.add_argument("--input", default=None, help="Input root (e.g., s3a://bucket/data or local).")
    parser.add_argument("--outdir", default="data/output", help="Output directory.")
    parser.add_argument("--skip-spark", action="store_true", help="Skip Spark; regenerate charts from existing CSVs.")
    ...
    if not args.skip_spark:
        ...
        # Extract application_id from file path: application_1485248649253_0001
        logs = logs.withColumn(
            "application_id",
            regexp_extract(col("file_path"), r"(application_\d+_\d+)", 1)
        )

        # Extract cluster_id (the first number): 1485248649253
        logs = logs.withColumn(
            "cluster_id",
            regexp_extract(col("application_id"), r"application_(\d+)_\d+", 1)
        )

        # Extract app number (zero-padded)
        logs = logs.withColumn(
            "app_number_raw",
            regexp_extract(col("application_id"), r"application_\d+_(\d+)", 1)
        ).withColumn("app_number", lpad(col("app_number_raw"), 4, "0"))

        # Extract timestamp from each line & convert to timestamp
        logs = logs.withColumn(
            "ts_str",
            regexp_extract(col("value"), r"^(\d{2}/\d{2}/\d{2} \d{2}:\d{2}:\d{2})", 1)
        ).withColumn("ts", to_timestamp(col("ts_str"), "yy/MM/dd HH:mm:ss"))

        # Keep only rows with parsed ids and timestamps
        parsed = logs.where(
            (col("application_id") != "") & (col("cluster_id") != "") & col("ts").isNotNull()
        ...
        # Compute duration in seconds for plotting convenience (in pandas after collect)
        ...
        timeline_pdf["duration_seconds"] = (timeline_pdf["end_time"] - timeline_pdf["start_time"]).dt.total_seconds().clip(lower=0)

        # Sort nicely
        timeline_pdf = timeline_pdf.sort_values(["cluster_id", "start_time", "application_id"])
        timeline_pdf[["cluster_id","application_id","app_number","start_time","end_time","duration_seconds"]] \
        ...
        grp = grp.sort_values("num_applications", ascending=False)
        ...
        # Only make the charts from existing CSVs (fast path)
        ...
        # Re-create stats (in case someone deleted the txt)
        write_stats_text(
            stats_txt,
            num_clusters=grp["cluster_id"].nunique(),
            num_apps=int(grp["num_applications"].sum()),
